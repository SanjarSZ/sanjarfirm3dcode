...
class Testing(unittest.TestCase):
    ...
    def test_out_of_bounds(self):
        ...
        xpoints = np.random.uniform(
            low=xran[1] + 0.1, high=xran[1] + 0.3, size=(nsamples,)
        )
        ypoints = np.random.uniform(
            low=yran[1] + 0.1, high=yran[1] + 0.3, size=(nsamples,)
        )
        zpoints = np.random.uniform(
            low=zran[1] + 0.1, high=zran[1] + 0.3, size=(nsamples,)
        )
        ...
        interpolant.interpolate_batch(fun)
        interpolant.evaluate_batch(xyz, fhxyz)
        assert np.allclose(fhxyz, 1.0, atol=1e-14, rtol=1e-14)
    ...
    def test_skip(self):
        ...
        interpolant = sopp.RegularGridInterpolant3D(
            rule, xran, yran, zran, dim, True, skip
        )
        interpolant.interpolate_batch(fun)
