...
class EigModeASCI:
    ...
    def load_data(self):
        ...
        self.modes = np.array(
            [(next(it), next(it)) for _ in range(self.num_fourier_modes)],
        ...
        self.egn_vectors = np.array(
            [
                ...
                for _ in range(
                    self.num_eigenmodes
                    * self.num_radial_points
                    * self.num_fourier_modes
                )
            ]
    ...
    def get_nearest_eigenvector(self, target_eigenvalue):
        ...
        data = [
            ...
            for I in range(len(self.egn_values))
        ]
        ...
        normalized_egn_vector = (
            egn_vector_sorted
            / egn_vector_sorted[np.argmax(np.abs(egn_vector_sorted[:, 0])), 0]
        )
    ...
    def from_eig_mode_asci(eig_mode_asci: EigModeASCI, target_eigenvalue: float):
        ...
        harmonics = [
            Harmonic(
                m=modes_sorted["m"][i],
                n=modes_sorted["n"][i],
                amplitudes=egn_vector_sorted[:, i],
            )
...
def plot_ae3d_eigenmode(mode: AE3DEigenvector, harmonics: int = 5):
    ...
    for i in range(num_harmonics_to_plot):
        ...
        fig.add_trace(
            go.Scatter(
                x=mode.s_coords,
                y=harmonic.amplitudes,
                mode="lines",
                name=f"(m={harmonic.m}, n={harmonic.n})",
            )
        )
    ...
    for evalue in ae3d.egn_values:
        ...
        if (m, n) in mn_set:
            for mode in ModeList:
                if (mode.get_poloidal_mode() == m) and (mode.get_toroidal_mode() == n):
                    break
            mode._s.append(
                evec.s_coords[
                    np.where(
                        evec.harmonics[0].amplitudes
                        == np.max(evec.harmonics[0].amplitudes)
                    )[0][0]
                ]
            )
